...
#if ((defined(USB_DEVICE_CONFIG_HOBART_BOOTLOADER)) && (USB_DEVICE_CONFIG_HOBART_BOOTLOADER > 0U))
#include "usbBootloader.h"


static usb_status_t USB_DeviceBootloaderAllocateHandle( usb_device_bootloader_t **pHandle );
static usb_status_t USB_DeviceBootloaderFreeHandle( usb_device_bootloader_t *pHandle );
static usb_status_t USB_DeviceBootloaderInterruptIn( usb_device_handle handle,
                                                  usb_device_endpoint_callback_message_struct_t *message,
                                                  void *callbackParam );
static usb_status_t USB_DeviceBootloaderInterruptOut( usb_device_handle handle,
                                                   usb_device_endpoint_callback_message_struct_t *message,
                                                   void *callbackParam );
static usb_status_t USB_DeviceBootloaderBulkOut( usb_device_handle handle,
                                              usb_device_endpoint_callback_message_struct_t *message,
                                              void *callbackParam );

static usb_status_t USB_DeviceBootloaderEndpointsInit( usb_device_bootloader_t *pHandle );
static usb_status_t USB_DeviceBootloaderEndpointsDeinit( usb_device_bootloader_t *pHandle );


USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static usb_device_bootloader_t
    s_UsbDeviceBootloaderHandle[USB_DEVICE_CONFIG_HOBART];


/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderAllocateHandle( usb_device_bootloader_t **pHandle )

      \brief
        This function allocates a device hobart bootloader class handle.

      \param pHandle    is out parameter, is used to return pointer of the hobart
                        bootloader class handle to the caller.
      \author
          Aaron Swift
*******************************************************************************/
static usb_status_t USB_DeviceBootloaderAllocateHandle( usb_device_bootloader_t **pHandle )
{
    ...
    {
        if (NULL == s_UsbDeviceBootloaderHandle[count].deviceHandle)
        {
            *pHandle = &s_UsbDeviceBootloaderHandle[count];
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderFreeHandle( usb_device_bootloader_t *pHandle )

      \brief
        This function frees a hobart Bootloader class handle.

      \param pHandle    hobart Bootloader class handle.
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderInterruptIn( usb_device_handle handle,
                                                           usb_device_endpoint_callback_message_struct_t *message,
                                                           void *callbackParam )

      \brief
        This function is the Bootloader interrupt in endpoint callback function
      ...
      \param callbackParam  The parameter for this callback. It is same with
                            ...
                            In the class, the value is the Bootloader class handle.
...
static usb_status_t USB_DeviceBootloaderInterruptIn( usb_device_handle handle,
                                                  ...
                                                  void *callbackParam )
{
    usb_device_bootloader_t *pPHandle;
    usb_status_t status = kStatus_USB_Error;

    /* get the hobart Bootloader class handle */
    pPHandle = (usb_device_bootloader_t *)callbackParam;
    ...
    if ((NULL != pPHandle->configStruct) && (NULL != pPHandle->configStruct->classCallback))
    {
        ...
        status =
            pPHandle->configStruct->classCallback((class_handle_t)pPHandle, kUSB_DeviceBootloaderEventSendResponse, message);
    }
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderInterruptOut( usb_device_handle handle,
                                                           usb_device_endpoint_callback_message_struct_t *message,
                                                           void *callbackParam )

      \brief
        This function is the Bootloader interrupt out endpoint callback function
...
static usb_status_t USB_DeviceBootloaderInterruptOut( usb_device_handle handle,
                                                   ...
                                                   void *callbackParam )
{
    usb_device_bootloader_t *pHandle;
    usb_status_t status = kStatus_USB_Error;

    /* get the hobart Bootloader class handle */
    pHandle = (usb_device_bootloader_t *)callbackParam;
    ...
    if ((NULL != pHandle->configStruct) && (NULL != pHandle->configStruct->classCallback))
    {
        /* notify the application data sent by calling the hobart class callback. classCallback is initialized
           in classInit of s_UsbDeviceClassInterfaceMap,it is from the second parameter of classInit */
        status =
            pHandle->configStruct->classCallback((class_handle_t)pHandle, kUSB_DeviceBootloaderEventRecvResponse, message);
    }
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderBulkOut( usb_device_handle handle,
                                                        usb_device_endpoint_callback_message_struct_t *message,
                                                        void *callbackParam )

      \brief
        This function is the Bootloader bulk out endpoint callback function
                                              ...
                                              void *callbackParam )
{
    ...
    pHandle->bulkOutPipeBusy = 0;
    if ((NULL != pHandle->configStruct) && (NULL != pHandle->configStruct->classCallback))
    {
        /* notify the application data sent by calling the hobart class callback. classCallback is initialized
           in classInit of s_UsbDeviceClassInterfaceMap,it is from the second parameter of classInit */
        status =
            pHandle->configStruct->classCallback((class_handle_t)pHandle, kUSB_DeviceBootloaderEventRecvBulkResponse, message);
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderEndpointsInit( usb_device_bootloader_t *pHandle )

      \brief
        This function initializes the endpoints of the hobart class

      \param pHandle  The hobart bootloader class handle. It equals the value 
...
static usb_status_t USB_DeviceBootloaderEndpointsInit( usb_device_bootloader_t *pHandle )
{
    ...
    {
        if (USB_DEVICE_CONFIG_HOBART_CLASS_CODE == interfaceList->interfaces[count].classCode)
        {
    ...
    for (count = 0U; count < interface->endpointList.count; count++)
    {
        ...
        /* endpoint list messed up, set bulk to 512 bytes not 64! */
        if( epInitStruct.endpointAddress == 3 ) {
            epInitStruct.maxPacketSize   = 512;
        } else {
        ...
        {
            epCallback.callbackFn          = USB_DeviceBootloaderBulkOut;
            pHandle->bulkOutPipeBusy       = 0U;
            pHandle->bulkOutPipeDataBuffer = (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
            pHandle->bulkOutPipeStall      = 0U;
            pHandle->bulkOutPipeDataLen    = 512U; /* was 0 */
            
        } else {
            epCallback.callbackFn                 = USB_DeviceBootloaderInterruptOut;
            pHandle->interruptOutPipeDataBuffer = (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
            pHandle->interruptOutPipeStall      = 0U;
            pHandle->interruptOutPipeDataLen    = 64U; /* was 0 */          
        }
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DeviceBootloaderEndpointsDeinit( usb_device_bootloader_t *pHandle )

      \brief
        This function de-initializes the endpoints of the hobart bootloader class.
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBBootloaderEvent( void *handle, uint32_t event, void *param )

      \brief
        This function handles the event passed to the hobart bootloader class.
      ...
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBBootloaderEvent( void *handle, uint32_t event, void *param )
{
    usb_device_bootloader_t *pHandle;
    ...
    pHandle = (usb_device_bootloader_t *)handle;
    ...
    switch (eventCode)
    {
        ...
        case kUSB_DeviceClassEventSetConfiguration:
            ...
            if (0U != pHandle->configuration)
            {
                error = USB_DeviceBootloaderEndpointsDeinit(pHandle);
            }
            ...
            error = USB_DeviceBootloaderEndpointsInit(pHandle);
            break;
        ...
        case kUSB_DeviceClassEventSetEndpointHalt:
            ...
            for (count = 0U; count < pHandle->interfaceHandle->endpointList.count; count++)
            {
                if( *temp8 == pHandle->interfaceHandle->endpointList.endpoint[count].endpointAddress )
                {
                    ...
                    {
                        if( pHandle->bulkOutEp == *temp8 ) {
                            pHandle->bulkOutPipeStall = 1U;
                        } else {
                        ...
                        if (0U != pHandle->interruptInPipeStall)
                        {
                            ...
                            if ((uint8_t *)USB_INVALID_TRANSFER_BUFFER != pHandle->interruptInPipeDataBuffer)
                            {
                                ...
                                if (kStatus_USB_Success != error)
                                {
                                    ...
                                    (void)USB_DeviceBootloaderInterruptIn(pHandle->deviceHandle, (void *)&endpointCallbackMessage,
                                                                   handle);
                                }
                                pHandle->interruptInPipeDataBuffer = (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
                        ...
                        if (0U != pHandle->interruptOutPipeStall)
                        {
                            ...
                            if ((uint8_t *)USB_INVALID_TRANSFER_BUFFER != pHandle->interruptOutPipeDataBuffer)
                            {
                                ...
                                if (kStatus_USB_Success != error)
                                {
                                    ...
                                    endpointCallbackMessage.isSetup = 0U;
                                    (void)USB_DeviceBootloaderInterruptOut(pHandle->deviceHandle, (void *)&endpointCallbackMessage,
                                                                    handle);
                                }
                                pHandle->interruptOutPipeDataBuffer = (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
        ...
        {
            /* handle the hobart bootloader class specific request. */
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBBootloaderInit( uint8_t controllerId, 
                                                usb_device_class_config_struct_t *config, 
                                                class_handle_t *handle )

      \brief
        This function is used to initialize the hobart bootloader class.

      \param controllerId  The controller id of the USB IP.
                             returned from usb_device_class_config_struct_t::classHandle.
      \param config   The class configuration information.                        
      \param handle   used to return pointer of the hobart bootloader class handle 
                        to the caller. 
                 
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBBootloaderInit(uint8_t controllerId, usb_device_class_config_struct_t *config, class_handle_t *handle)
{
    usb_device_bootloader_t *pHandle;
    usb_status_t error;

    /* allocate a hobart bootloader class handle. */
    error = USB_DeviceBootloaderAllocateHandle(&pHandle);
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBBootloaderDeinit( class_handle_t handle )

      \brief
        This function is used to de-initialize the hobart bootloader class.
                       
      \param handle   hobart bootloader class handle 

                 
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBBootloaderDeinit(class_handle_t handle)
{
    usb_device_bootloader_t *pHandle;
    usb_status_t error;
    ...
    (void)USB_DeviceBootloaderFreeHandle(pHandle);
    return error;
}


/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBBootloaderSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length)

      \brief
        This function is used to send data through a specified endpoint.
                       
      \param handle     The hobart bootloader class handle.
      ...
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBBootloaderSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length)
{
    usb_device_bootloader_t *pHandle;
    usb_status_t error = kStatus_USB_Error;
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBBootloaderRecv( class_handle_t handle, uint8_t ep, 
      ...
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBBootloaderRecv( class_handle_t handle, uint8_t ep, 
                                      uint8_t *buffer, uint32_t length )
{
    usb_device_bootloader_t *pHandle;
    usb_status_t error;
    ...
    /* determine which endpoint interrupt or bulk */
    if( ep == 1 ) 
    {
        ...
        if (0U != pHandle->bulkOutPipeBusy)
        {
            return kStatus_USB_Busy;
        }
        pHandle->bulkOutPipeBusy = 1U;

        if (0U != pHandle->bulkOutPipeStall)
        {
            pHandle->bulkOutPipeDataBuffer = buffer;
            pHandle->bulkOutPipeDataLen    = length;
            return kStatus_USB_Success;
        }
        error = USB_DeviceRecvRequest( pHandle->deviceHandle, ep, buffer, length );
        if (kStatus_USB_Success != error)
