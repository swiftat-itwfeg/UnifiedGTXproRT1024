...
{ 
    ...
    PR_STOP_TIME,
    PR_LABEL_SIZE,
    PR_REQ_CUTTER_VERSION,
    PR_CUTTER_VERSION,
    PR_REQ_GBL_CUTTER_STATUS,
    PR_GL_CUTTER_STATUS,
...
typedef struct 
{
    ...
    unsigned short      takeup_reading;
    unsigned short      labelTakenReading;
}GPrSensors;
...
#define MAX_PROD_NAME_SIZE                      20
#define MAX_FIRM_NUM_SIZE                       12
#define MAX_FIRM_ISSUE_SIZE                     8
#define MAX_DATE_SZ                             11
#define MAX_TIME_SZ                             8

typedef struct
{
    PRMsgType	        msgType;
    unsigned char       productName[ ( MAX_PROD_NAME_SIZE + 1 )];
    unsigned char       firmware[ ( MAX_FIRM_NUM_SIZE + 1 )];
    unsigned char       issueDate[ ( MAX_FIRM_ISSUE_SIZE + 1 ) ];
    unsigned char       date[ ( MAX_DATE_SZ + 1 ) ];
    unsigned char       time[ ( MAX_TIME_SZ + 1 ) ];  
}PrCutterVersion;
...
{
    PRMsgType                msgType;       
    uint16_t                 startingDotIndex;   
    char                     reserved[2];       
    unsigned char            dots[56];  
}PrDotStatusMessage;          
/* Printer Messages */
typedef union 
{
    ...
    PrLowLabel          lowLabel;
    PrCutterVersion     cutterVersion;
    PrGblCutterStatus   gCutterStatus;
    PrDotStatusMessage  dotStatus;
}PrMessage;
