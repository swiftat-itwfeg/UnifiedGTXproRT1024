...
BaseType_t createGlobalPrinterTask( PrinterStyle style, QueueHandle_t msgQueue )
{
	...
	*	the first time.
	*/
	if( config_.takeupDriveCurrent >= CC_TWENTY_FIVE_POINT_FIVE )
	{
		PRINTF("createGlobalPrinterTask(): TU Drive current is Maxed!\r\n");
		PRINTF("createGlobalPrinterTask(): config_.takeupDriveCurrent: %d\r\n", config_.takeupDriveCurrent );
		config_.takeupDriveCurrent 		= CC_SEVEN_POINT_FIVE;
		config_.takeupMaxTension	= MIN_TU_CAL_DELTA_CNTS;
		config_.takeupMinTension	= MIN_TU_CAL_DELTA_CNTS;
		PRINTF("createGlobalPrinterTask(): Setting TU Drive current to:\r\n");
		PRINTF("createGlobalPrinterTask(): config_.takeupDriveCurrent: %d\r\n", config_.takeupDriveCurrent );
		PRINTF("createGlobalPrinterTask(): config_.takeupMaxTension: %d\r\n", config_.takeupMaxTension );
		PRINTF("createGlobalPrinterTask(): config_.takeupMinTension: %d\r\n", config_.takeupMinTension );
	}
    ...
    switch( pMsg->generic.msgType )
    {
        ...
        {          
            ...
            if( pMsg->config.disposition == DEFAULT_CFG ) {
                ...
                //pMsg->config.config.shootThroughCal = config_.shootThroughCal;
        ...
        case PR_FACTORY_DFLTS:
        {
            ...
            if( setSerialPrDfltConfiguration( &config_ ) ) 
            {
                ...
                pPrConfig->takeupDriveCurrent      = 62;
                pPrConfig->takeupMaxTension = 0;
                pPrConfig->takeupMinTension = 0;
                */
        ...
        {         
            ...
            if( gapSensorCal( step_, &calPoint ) ) {
                ...
                if( step_ == _CALBACKING )  
                    ...
                    config_.shootThroughCal = calPoint;
...
static bool verifyPrConfiguration( Pr_Config *pTemp, Pr_Config *pSF )
{
    ...
    if( memcmp( pTemp, pSF, sizeof(Pr_Config) ) == 0 ) {
        ...
        PRINTF("shootThroughCal: %d  %d\r\n", pTemp->shootThroughCal, pSF->shootThroughCal );
        ...
        PRINTF("takeupDriveCurrent: %d  %d\r\n", pTemp->takeupDriveCurrent, pSF->takeupDriveCurrent );
        PRINTF("takeupMaxTension: %d  %d\r\n", pTemp->takeupMaxTension, pSF->takeupMaxTension );
        PRINTF("takeupMinTension: %d  %d\r\n", pTemp->takeupMinTension, pSF->takeupMinTension );
        ...
        if( pTemp->shootThroughCal != pSF->shootThroughCal ) {
            PRINTF("shootThroughCal: %d  %d\r\n", pTemp->shootThroughCal, pSF->shootThroughCal );          
        }
        ...
        if( pTemp->takeupDriveCurrent != pSF->takeupDriveCurrent ) {
            PRINTF("takeupDriveCurrent: %d  %d\r\n", pTemp->takeupDriveCurrent, pSF->takeupDriveCurrent );   
        }
        if( pTemp->takeupMaxTension != pSF->takeupMaxTension ) {
            PRINTF("takeupMaxTension: %d  %d\r\n", pTemp->takeupMaxTension, pSF->takeupMaxTension );  
        }
        if( pTemp->takeupMinTension != pSF->takeupMinTension ) {
            PRINTF("takeupMinTension: %d  %d\r\n", pTemp->takeupMinTension, pSF->takeupMinTension );
        }       
    }
    return result;
...
{
    config_.takeupDriveCurrent = val;
    config_.takeupMaxTension = max;
    config_.takeupMinTension = min;
}
