...
void initializeQueueManager( PeripheralModel_t model )
{
    ...
    if( ( model == GLOBAL_SCALE_HB_GT ) || ( model == GLOBAL_SCALE_HB_DT ) ) {
        ...
        if( ( model == GLOBAL_SCALE_AV_XPRO ) || ( model == GLOBAL_SCALE_AV_XONE ) ) {
            ...
            /* create Weigher message queue */
            pWgQueue_  = xQueueCreate( MAX_AVERY_MSG_LENGTH, MAX_AVERY_W_MSG_DEPTH );
            if( pWgQueue_ == NULL ) {
                PRINTF("initializeQueueManager(): Failed to create Weigher message queue!\r\n" );
            }
            /* create Weigher a/d counts queue */
            ...
            /* create Printer message queue */
            pPrQueue_  = xQueueCreate( MAX_AVERY_MSG_LENGTH, MAX_AVERY_P_MSG_DEPTH );
            if( pPrQueue_ == NULL ) {
                PRINTF("initializeQueueManager(): Failed to create Printer message queue!\r\n" );
            }            
        }
