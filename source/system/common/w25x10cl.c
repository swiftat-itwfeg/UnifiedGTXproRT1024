...
AT_NONCACHEABLE_SECTION( static SERIAL_FLASH_CNTRL serialFlash );
AT_NONCACHEABLE_SECTION(static lpspi_master_handle_t  spi1Handle_);
AT_NONCACHEABLE_SECTION(static lpspi_master_handle_t  spi4Handle_);

static SemaphoreHandle_t sfMutex_;
static SemaphoreHandle_t wfbMutex_;
...
bool initializeSerialFlash()
{
    ...
    sfMutex_ = xSemaphoreCreateMutex(); 
    if( !sfMutex_ ) {     
        PRINTF("initializeSerialFlash(): Failed to create sf mutex!\r\n" );
    }
    
    wfbMutex_ = xSemaphoreCreateRecursiveMutex(); 
    if( !wfbMutex_ ) {     
        PRINTF("initializeSerialFlash(): Failed to create wfb mutex!\r\n" );
    }
...
{
    bool status = true;
    if( xSemaphoreTake( sfMutex_, ( TickType_t )portMAX_DELAY ) == pdTRUE ) {
        while( status && (size > 0)) 
        {
            int sz;
        ...
        xSemaphoreGive( sfMutex_ );
    } else {
        PRINTF("readSerialFlash(): Failed to take mutex!\r\n" );
    }
    return status;       
}
...
bool writeSerialFlash( uint8_t *pData, uint32_t address, int size, FLASH_SECTORS sector )
{
    ...
    if( xSemaphoreTake( sfMutex_, ( TickType_t )portMAX_DELAY ) == pdTRUE ) {    
        ...
        PRINTF("writeSerialFlash(): Failed to take mutex!\r\n" );
    }
    return status;       
}
...
bool writeSerialFlashNE( uint8_t *pData, uint32_t address, int size, FLASH_SECTORS sector )
{
    ...
    if( xSemaphoreTake( sfMutex_, ( TickType_t )portMAX_DELAY ) == pdTRUE ) {    
        ...
        PRINTF("writeSerialFlashNE(): Failed to take mutex!\r\n" );
    }
    return status;           
}
...
bool eraseSector( uint32_t address, FLASH_SECTORS sector )
{
    ...
    {  
        ...
        PRINTF("eraseSector(): address >= SECTOR2_BASE_ADDR!\r\n" );
    }
    return status;           
...
/*! ****************************************************************************   
      \fn bool validateErasedPage( uint32_t address, FLASH_SECTORS sector )

      \brief
        public: This function verifies the page erase function has erased the 
                page (4KB) within the serial flash sector. 
      
      \param address  begining address of page to which sector.
      
      \return true: page has been erased  
...
bool validateErasedPage( uint32_t address, FLASH_SECTORS sector )
{
    ...
    while( ( cnt_ != 2 ) &&  !error ) {   
        address = address_;
        setupTransfer( FLASH_READ_DATA_BYTES_HS, &serialFlash,  (uint8_t *)&address,  &bfr[0], 256 );
        if( serialFlash.error == 0 ) {
            processCommand( &serialFlash );
            if( serialFlash.error == 0 ) {
                memcpy( &bfr[0], getReadData(0), 256 );
                for( int i = 0; i < 256; i++ ) {                
                    if( bfr[i] != 255 ) {                        
                        PRINTF("validateErasedPage(): failed at address: %d value: %d \r\n", ( address + i), bfr[i]  );  
                        error = true;
                        break;
                    }                
                }
                ...
                PRINTF("validateErasedPage(): failed to read at address: %d\r\n", address );   
            }          
        }
    }
    if( error ) {
                    ...
                    int length )
{
    ...
    switch(command)
    {
        ...
        {            
            ...
            pSerialFlash->transferBuffer[i++] = DUMMY_BYTE;     /* Read M7-M0 */
            pSerialFlash->transferBuffer[i++] = DUMMY_BYTE;     /* Read ID15-ID8 */
            pSerialFlash->transferBuffer[i++] = DUMMY_BYTE;     /* Read ID7-ID0 */
        ...
        {
            if( (length <= (BUFFER_SIZE - CMD_SIZE - ADDR_SIZE)) && (length <= WRITE_BLOCK_SIZE) ) {
                int j;
...
void processCommand(SERIAL_FLASH_CNTRL *pSerialFlash)
{
    switch(pSerialFlash->currentCommand)
    {
        ...
        case FLASH_READ_ES:
        {
            PRINTF("processCommand(): FLASH_READ_ES not supported!\r\n");
            break;
        }
        case FLASH_WRITE_EN:
        {
...
static void handleReadId(SERIAL_FLASH_CNTRL *pSerialFlash)
{
    ...
    if( xSemaphoreTakeRecursive( wfbMutex_, ( TickType_t )portMAX_DELAY ) == pdTRUE ) {
        writeFlashByte( );    
        if( pSerialFlash->error == 0 ) {
        ...
        xSemaphoreGiveRecursive( wfbMutex_ );
    }
}
...
/*! ****************************************************************************   
      ...
      \brief
        private: this function will write a byte to the serial flash
...
static void writeFlashByte()
{
    ...
    if( xSemaphoreTakeRecursive( wfbMutex_, ( TickType_t )portMAX_DELAY ) == pdTRUE ) {
        ...
        transfer.configFlags = ( (uint32_t)kLPSPI_Pcs0 | (uint32_t)kLPSPI_MasterPcsContinuous); 


        result = LPSPI_MasterTransferNonBlocking( LPSPI1, &spi1Handle_, &transfer );
        if( result != kStatus_Success) {
            PRINTF("writeFlashByte(): Transfer failure: %d !\r\n", result );
        }
        
        while( !serialFlash.dataReady ) {
            asm("nop");
        }
...
/*! ****************************************************************************   
      \fn void lpspi1Callback(LPSPI_Type *base, lpspi_master_handle_t *handle, 
                              status_t status, void *userData)                                                        
 
      \brief
        This function is the callback for the spu interface to the serial flash.
