...
bool gapSensorCal( GAPSteps step, unsigned char *pBias )
{
    ...
    switch( step )
    {
        ...
        case _CALBACKING: {
                ...
                checkForPaper( (uint16_t)((float)config_.takeupMaxTension * 0.80), 820 );
        ...
        case _CALRESULTS: {
            if( pBacking && pLabel && pDeflection ) {
                ...
                *pBias = ((float)*pBias); 
                ...
                calStatus.takeupDriveCurrent = (int)config_.takeupDriveCurrent;
...
/******************************************************************************/
/*!   \fn bool mediaSensorCal( GAPSteps step, unsigned char *pBias )
      ...
      \author
       Chris King
*******************************************************************************/
bool mediaSensorCal( GAPSteps step, unsigned char *pBias )
{
    bool result = false;
    static unsigned short *pBacking = NULL, *pLabel = NULL, *pDeflection = NULL;
    static unsigned short *pBackingS = NULL, *pLabelS = NULL, *pDeflectionS = NULL;
    ...
    {
        case _INIT: {
            PRINTF("mediaSensorCal(): _INIT HT\r\n");  
        ...
        case _CALBACKING: {
                if( pBacking && pLabel && pDeflection ) 
                {
                PRINTF("mediaSensorCal(): _CALBACKING start HT\r\n");
               
                /* pause the sensors thread from doing conversions */
                pauseResumeConversions( true );

                /* set media sensor duty cyle to min */
                setMediaSensorDutyCycle(0);
                ...
                *pBacking = getMediaCounts();  
                ...
                for(uint8_t pwmDutyCycle = 1; pwmDutyCycle < 101; pwmDutyCycle++)
                {
                    delay_uS(110);
                    setMediaSensorDutyCycle(pwmDutyCycle);
                    *pBacking = getShootAverage();
                    
                    PRINTF("\r\npBacking %d %d", pwmDutyCycle, *pBacking);
        ...
        case _CALLABEL: {
            if( pBacking && pLabel && pDeflection ) 
            {
                PRINTF("mediaSensorCal(): _CALLABEL HT\r\n");
                ...
                *pLabel = getMediaCounts();  
                ...
                for(uint8_t pwmDutyCycle = 1; pwmDutyCycle < 101; pwmDutyCycle++)
                {
                    ...
                    *pLabel = getShootAverage();
                    ...
                    PRINTF("\r\npLabel %d %d", pwmDutyCycle, *pLabel);
        ...
        case _CALRESULTS: {
            if( pBacking && pLabel && pDeflection ) {
            PRINTF("mediaSensorCal(): _CALRESULTS HT\r\n");
                /* calculate deflections */
                for( int i = 0; i < CC_TWENTY_FIVE_POINT_FIVE; i++ ) {
                    *pDeflection = abs( *pLabel - *pBacking );
                    /*
                ...
                static unsigned short *pBackingPrev = NULL;
                pBackingPrev = pBacking;
                ...
                for(uint8_t duty = 9; duty < 100; duty++)
                {
                    pBacking++;
                    pLabel++;
                  
                    PRINTF("\r\n(*pBackingPrev - *pBacking) %d", (*pBackingPrev - *pBacking));
                    
                    if((*pBackingPrev - *pBacking) <= 5)
                    {
                        if(calCounter <= 1)
                        {
                            calCounter++;
                            PRINTF("\r\npBacking target hit %d %d", duty, *pBacking);
                        }
                        else
                        {
                            *pBias = (duty);
                ...
                setMediaSensorDutyCycle(*pBias);

                PRINTF("Media sensor PWM duty: %d\r\n", *pBias);
                PRINTF("pBacking = %d\r\n", *pBacking);
                PRINTF("pLabel = %d\r\n\r\n", *pLabel);
                
                setConfigBackingValue( *pBacking );
                setConfigLabelValue( *pLabel );
        ...
        case _CALDONE: {
            PRINTF("mediaSensorCal(): _CALDONE HT\r\n");
            result = true;
            calStatus.state =_Done;        
...
bool TUSensorCal( GAPSteps step, unsigned char *pBias )
{
    ...
    switch( step )
    {
        ...
        case _CALRESULTS: 
        {
            ...
            calStatus.takeupDriveCurrent = TUDrive;
            calStatus.driveCurrent = config_.takeupDriveCurrent;
