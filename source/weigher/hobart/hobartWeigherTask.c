...
/******************************************************************************/
/*!   \fn BaseType_t createHobartWeigherTask( WeigherStyle style, QueueHandle_t msgQueue )                                                        
      ...
      \author
          Aaron Swift / Joseph 
*******************************************************************************/ 
BaseType_t createHobartWeigherTask( WeigherStyle style, QueueHandle_t msgQueue )
{
    BaseType_t result;
    ...
    PRINTF("createHobartWeigherTask(): value_max_on_off = %d\r\n", config.value_max_on_off);
	...
	*/
    ...
    if( initializeCs5530( adQHandle_ ) ) {    
        PRINTF("createHobartWeigherTask(): initializing CS5530()\r\n" );
        /* only best model has valuemax */ 
        if(config.value_max_on_off == true)
        {
            PRINTF("createHobartWeigherTask(): createValueMaxTask()\r\n" );
            /* spinup our accelerometer task */
            if( createValueMaxTask() != pdTRUE ) {            
                PRINTF("createHobartWeigherTask(): Failed to create accelerometer task!\r\n" );
            }
        }
        ...
        if( ( pMsgQHandle_ != NULL ) && ( adQHandle_ != NULL ) && 
           ( wQueueSet_ != NULL ) ) {
              ...
              PRINTF("createHobartWeigherTask(): Critical Error queue set not created!\r\n" );
           }        
        
        PRINTF("createHobartWeigherTask(): Starting...\r\n" );
        ...
        PRINTF("createHobartWeigherTask(): Failed to start external weigher a/d!\r\n" );
    }
    return result;
}
...
static void getSerialConfiguration( WgConfiguration *pCfg  )
{
    ...
    if( getSerialWgConfiguration( pCfg ) ) {
        ...
        PRINTF("createHobartWeigherTask(): failed to read configuration from serial flash!\r\n" ); 
    ...
    if( checkSum != sums.wgConfigSum ) {
        ...
        PRINTF("createHobartWeigherTask(): Setting default configuration!\r\n" );
        PRINTF("createHobartWeigherTask(): checksum = %d\r\n", checkSum);
        PRINTF("createHobartWeigherTask(): sums.wgConfigSum = %d\r\n", sums.wgConfigSum);
        ...
        if( !blankDevice ) {
            ...
            PRINTF("createHobartWeigherTask(): Default configuration loaded!\r\n" );
        } else {
            setPageChecksums( &sums );
        }
        configValid_ = true;
    } else {    	
        PRINTF("createHobartWeigherTask(): Configuration valid!\r\n" );        
        errorState_ = false;
        configValid_ = true;
    } 
    /* release the lock onthe serial flash */
