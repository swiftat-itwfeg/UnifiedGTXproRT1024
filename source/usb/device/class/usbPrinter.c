...
#if ((defined(USB_DEVICE_CONFIG_HOBART_PRINTER)) && (USB_DEVICE_CONFIG_HOBART_PRINTER > 0U))
#include "usbPrinter.h"


static usb_status_t USB_DevicePrinterAllocateHandle( usb_device_printer_t **pHandle );
static usb_status_t USB_DevicePrinterFreeHandle( usb_device_printer_t *pHandle );
static usb_status_t USB_DevicePrinterInterruptIn( usb_device_handle handle,
                                                  usb_device_endpoint_callback_message_struct_t *message,
                                                  void *callbackParam );
static usb_status_t USB_DevicePrinterInterruptOut( usb_device_handle handle,
                                                   usb_device_endpoint_callback_message_struct_t *message,
                                                   void *callbackParam );
static usb_status_t USB_DevicePrinterBulkOut( usb_device_handle handle,
                                              usb_device_endpoint_callback_message_struct_t *message,
                                              void *callbackParam );

static usb_status_t USB_DevicePrinterEndpointsInit( usb_device_printer_t *pHandle );
static usb_status_t USB_DevicePrinterEndpointsDeinit( usb_device_printer_t *pHandle );


USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static usb_device_printer_t
    s_UsbDevicePrinterHandle[USB_DEVICE_CONFIG_HOBART];


/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterAllocateHandle( usb_device_printer_t **pHandle )

      \brief
        This function allocates a device hobart printer class handle.

      \param pHandle    is out parameter, is used to return pointer of the hobart
                        printer class handle to the caller.
      \author
          Aaron Swift
*******************************************************************************/
static usb_status_t USB_DevicePrinterAllocateHandle( usb_device_printer_t **pHandle )
{
    ...
    {
        if (NULL == s_UsbDevicePrinterHandle[count].deviceHandle)
        {
            *pHandle = &s_UsbDevicePrinterHandle[count];
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterFreeHandle( usb_device_printer_t *pHandle )

      \brief
        This function frees a hobart printer class handle.

      \param pHandle    hobart printer class handle.
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterInterruptIn( usb_device_handle handle,
                                                           usb_device_endpoint_callback_message_struct_t *message,
                                                           void *callbackParam )

      \brief
        This function is the printer interrupt in endpoint callback function
...
static usb_status_t USB_DevicePrinterInterruptIn( usb_device_handle handle,
                                                  ...
                                                  void *callbackParam )
{
    usb_device_printer_t *pPHandle;
    usb_status_t status = kStatus_USB_Error;

    /* get the hobart printer class handle */
    pPHandle = (usb_device_printer_t *)callbackParam;
    ...
    if ((NULL != pPHandle->configStruct) && (NULL != pPHandle->configStruct->classCallback))
    {
        ...
        status =
            pPHandle->configStruct->classCallback((class_handle_t)pPHandle, kUSB_DevicePrinterEventSendResponse, message);
    }
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterInterruptOut( usb_device_handle handle,
                                                           usb_device_endpoint_callback_message_struct_t *message,
                                                           void *callbackParam )

      \brief
        This function is the printer interrupt out endpoint callback function
...
static usb_status_t USB_DevicePrinterInterruptOut( usb_device_handle handle,
                                                   ...
                                                   void *callbackParam )
{
    usb_device_printer_t *pHandle;
    usb_status_t status = kStatus_USB_Error;

    /* get the hobart printer class handle */
    pHandle = (usb_device_printer_t *)callbackParam;
    ...
    if ((NULL != pHandle->configStruct) && (NULL != pHandle->configStruct->classCallback))
    {
        /* notify the application data sent by calling the hobart class callback. classCallback is initialized
           in classInit of s_UsbDeviceClassInterfaceMap,it is from the second parameter of classInit */
        status =
            pHandle->configStruct->classCallback((class_handle_t)pHandle, kUSB_DevicePrinterEventRecvResponse, message);
    }
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterBulkOut( usb_device_handle handle,
                                                        usb_device_endpoint_callback_message_struct_t *message,
                                                        void *callbackParam )

      \brief
        This function is the printer bulk out endpoint callback function
                                              ...
                                              void *callbackParam )
{
    ...
    if ((NULL != pHandle->configStruct) && (NULL != pHandle->configStruct->classCallback))
    {
        /* notify the application data sent by calling the hobart class callback. classCallback is initialized
           in classInit of s_UsbDeviceClassInterfaceMap,it is from the second parameter of classInit */
        status =
            pHandle->configStruct->classCallback((class_handle_t)pHandle, kUSB_DevicePrinterEventRecvBulkResponse, message);
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterEndpointsInit( usb_device_printer_t *pHandle )

      \brief
        This function initializes the endpoints of the hobart class

      \param pHandle  The hobart printer class handle. It equals the value 
...
static usb_status_t USB_DevicePrinterEndpointsInit( usb_device_printer_t *pHandle )
{
    ...
    for (count = 0U; count < interface->endpointList.count; count++)
    {
        ...
        {
            epCallback.callbackFn                = USB_DevicePrinterInterruptIn;
        ...
        {
            epCallback.callbackFn          = USB_DevicePrinterBulkOut;
            ...
            epCallback.callbackFn                 = USB_DevicePrinterInterruptOut;
...
/******************************************************************************/
/*!   \fn static usb_status_t USB_DevicePrinterEndpointsDeinit( usb_device_printer_t *pHandle )

      \brief
        This function de-initializes the endpoints of the hobart printer class.
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBPrinterEvent( void *handle, uint32_t event, void *param )

      \brief
        This function handles the event passed to the hobart printer class.
      ...
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBPrinterEvent( void *handle, uint32_t event, void *param )
{
    usb_device_printer_t *pHandle;
    ...
    pHandle = (usb_device_printer_t *)handle;
    ...
    switch (eventCode)
    {
        ...
        case kUSB_DeviceClassEventSetConfiguration:
            ...
            if (0U != pHandle->configuration)
            {
                error = USB_DevicePrinterEndpointsDeinit(pHandle);
            }
            ...
            error = USB_DevicePrinterEndpointsInit(pHandle);
            break;
                        ...
                        if (0U != pHandle->interruptInPipeStall)
                        {
                            ...
                            if ((uint8_t *)USB_INVALID_TRANSFER_BUFFER != pHandle->interruptInPipeDataBuffer)
                            {
                                ...
                                if (kStatus_USB_Success != error)
                                {
                                    ...
                                    (void)USB_DevicePrinterInterruptIn(pHandle->deviceHandle, (void *)&endpointCallbackMessage,
                                                                   handle);
                                }
                                pHandle->interruptInPipeDataBuffer = (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
                        ...
                        if (0U != pHandle->interruptOutPipeStall)
                        {
                            ...
                            if ((uint8_t *)USB_INVALID_TRANSFER_BUFFER != pHandle->interruptOutPipeDataBuffer)
                            {
                                ...
                                if (kStatus_USB_Success != error)
                                {
                                    ...
                                    endpointCallbackMessage.isSetup = 0U;
                                    (void)USB_DevicePrinterInterruptOut(pHandle->deviceHandle, (void *)&endpointCallbackMessage,
                                                                    handle);
                                }
                                pHandle->interruptOutPipeDataBuffer = (uint8_t *)USB_INVALID_TRANSFER_BUFFER;
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBPrinterInit( uint8_t controllerId, 
                                                usb_device_class_config_struct_t *config, 
                                                class_handle_t *handle )

      \brief
        This function is used to initialize the hobart printer class.

      \param controllerId  The controller id of the USB IP.
                             returned from usb_device_class_config_struct_t::classHandle.
      \param config   The class configuration information.                        
      \param handle   used to return pointer of the hobart printer class handle 
                        to the caller. 
                 
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBPrinterInit(uint8_t controllerId, usb_device_class_config_struct_t *config, class_handle_t *handle)
{
    usb_device_printer_t *pHandle;
    usb_status_t error;

    /* allocate a hobart printer class handle. */
    error = USB_DevicePrinterAllocateHandle(&pHandle);
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBPrinterDeinit( class_handle_t handle )

      \brief
        This function is used to de-initialize the hobart printer class.
                       
      \param handle   hobart printer class handle 

                 
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBPrinterDeinit(class_handle_t handle)
{
    usb_device_printer_t *pHandle;
    usb_status_t error;
    ...
    (void)USB_DevicePrinterFreeHandle(pHandle);
    return error;
}


/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBPrinterSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length)
      ...
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBPrinterSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length)
{
    usb_device_printer_t *pHandle;
    usb_status_t error = kStatus_USB_Error;
...
/******************************************************************************/
/*!   \fn usb_status_t USB_DeviceHBPrinterRecv( class_handle_t handle, uint8_t ep, 
      ...
      \author
          Aaron Swift
*******************************************************************************/
usb_status_t USB_DeviceHBPrinterRecv( class_handle_t handle, uint8_t ep, 
                                      uint8_t *buffer, uint32_t length )
{
    usb_device_printer_t *pHandle;
    usb_status_t error;
