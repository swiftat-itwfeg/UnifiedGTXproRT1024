...
typedef enum _usb_device_weigher_event
{
    kUSB_DeviceWeigherEventSendResponse = 0x01U, /*!< Send data completed or cancelled etc*/
    kUSB_DeviceWeigherEventRecvResponse,         /*!< Data received or cancelled etc*/
    ...
    kUSB_DeviceWeigherEventSetProtocol,          /*!< Set protocol request */
    //kUSB_DevicePrinterEventRequestReportBuffer,  /*!< Get buffer to save the data of the set report request. */
} usb_device_weigher_event_t;

typedef struct _usb_device_weigher
{
...
/*!
 * @brief Initializes the Hobart Weigher class.
 *
 * This function is used to initialize the Hobart Weigher class. This function only can be called by #USB_DeviceClassInit.
 *
 ...
 */
extern usb_status_t USB_DeviceHBWeigherInit(uint8_t controllerId,
                                      usb_device_class_config_struct_t *config,
                                      class_handle_t *handle);

/*!
 * @brief Deinitializes the device Hobart Weigher class.
 *
 * The function deinitializes the device Hobart Weigher class. This function only can be called by #USB_DeviceClassDeinit.
 *
 * @param[in] handle The Hobart Weigher class handle got from usb_device_class_config_struct_t::classHandle.
 *
 * @return A USB error code or kStatus_USB_Success.
 */
extern usb_status_t USB_DeviceHBWeigherDeinit(class_handle_t handle);

/*!
 * @brief Handles the event passed to the Hobart Weigher class.
 *
 * This function handles the event passed to the Hobart Weigher class. This function only can be called by #USB_DeviceClassEvent.
 *
 * @param[in] handle          The Hobart Weigher class handle received from the usb_device_class_config_struct_t::classHandle.
 ...
 */
extern usb_status_t USB_DeviceHBWeigherEvent(void *handle, uint32_t event, void *param);

/*!
 * @name USB device Hobart Weigher class APIs
 * @{
 */
 ...
 *
 * @param[in] handle The Hobart Weigher class handle received from usb_device_class_config_struct_t::classHandle.
 ...
 * callback).
 */
extern usb_status_t USB_DeviceHBWeigherSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length);
 ...
 * callback).
 */
extern usb_status_t USB_DeviceHBWeigherRecv(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t length);
